...
RSpec.describe 'タスク管理機能', type: :system do
  ...
  describe '新規作成機能' do
    context 'タスクを新規作成した場合ステータスも登録できる' do
      it '作成したタスクが表示される' do
        task = FactoryBot.create(:task, title: 'sample1',content: 'sample1',status: '未着手', user: user)
        visit new_task_path
  ...
  describe '一覧表示機能' do
    context '一覧画面に遷移した場合' do
      it '作成済みのタスク一覧が表示される' do
        task = FactoryBot.create(:task, title: 'sample1',user: user)
        visit tasks_path
        expect(page).to have_content 'sample1'
      end
    end
  ...
  describe '詳細表示機能' do
    context '任意のタスク詳細画面に遷移した場合' do
      it '該当タスクの内容が表示される' do
        task = FactoryBot.create(:task, title: 'sample1',content: 'sample1', user: user)
        visit task_path(task.id)
        expect(page).to have_content 'sample1'
      end
      it '該当タスクのラベルが表示されている' do
        task = FactoryBot.create(:task, title: 'sample1',content: 'sample1',  user: user)
        visit task_path(task.id)
        expect(page).to have_content 'sample_label'
      end
